...
const filters = [
  "All",
  ...new Set(gadgetProducts.map((product) => product.category)),
];
const sortOptions = ["Default", "Low to High", "High to Low"];

const whatsappNumber = "+919496391811"; // Your number here
  ...
  const filteredGadgets = gadgetProducts
    ...
    .filter((product) => {
      ...
      const titleMatch = product.title
        ?.toLowerCase()
        .includes(combinedQuery.toLowerCase());
      const numberMatch = product.productNumber
        ?.toString()
        ...
        {visibleProducts.length > 0 ? (
          visibleProducts.map((p, idx) => {
            ...
            const imageUrl = p.img.startsWith("http")
              ? p.img
              : `https://techstore-lime.vercel.app/${p.img}`;
            ...
            return (
              ...
              >
                <div
                  className="bg-white shadow-md rounded-sm overflow-hidden text-center relative cursor-pointer transition-shadow duration-300 hover:shadow-lg
    h-72 sm:h-80"
                >
                  {/* Product Image */}
                  <img
          ...
          >
            ...
            <div className="p-5">
              ...
              <p className="text-sm text-gray-500">
                Product No: #{selectedProduct.productNumber}
              </p>
